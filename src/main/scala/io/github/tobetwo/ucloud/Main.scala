...
import io.github.tobetwo.ucloud.actor.{Close, CommonActor}
...
object Main extends App {
  ...
  mainArgs.action match {
    case "start" =>
      system.actorOf(Props(s"io.github.tobetwo.ucloud.actor.${mainArgs.mode.headToUpper}Actor".as[CommonActor](ucloudConfig)), mainArgs.mode)
    case "stop" =>
      system.actorSelection(s"akka.tcp://UCloud@${ucloudConfig.UCLOUD_HOST}:${ucloudConfig.UCLOUD_PORT}/user/${mainArgs.mode}") ?
        Close onComplete
        (_ => system.terminate)
  }
}
